...
class SessionManager:
    ...
    async def route_order_to_best_session(self, symbol: str, side: str) -> dict[str, Any] | None:
        eligible_sessions = [session for session in self.list_sessions() if session.status in {"ready", "running"}]
        if not eligible_sessions:
            return None

        quotes = await asyncio.gather(
            *(session.route_price(symbol, side) for session in eligible_sessions),
        ...
        for quote in quotes:
            if isinstance(quote, Exception):
                self.logger.debug(
                    "Best-route quote failed",
                    exc_info=(type(quote), quote, quote.__traceback__),
                )
                continue
        ...
        if str(side or "").strip().lower() == "sell":
            candidates.sort(key=lambda row: float(row.get("price") or 0.0), reverse=True)
        else:
            candidates.sort(key=lambda row: float(row.get("price") or 0.0))
        return candidates[0]
